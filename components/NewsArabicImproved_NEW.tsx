...
import { translatePaperToArabic } from '../services/translationService';
...
const NewsArabicImproved: React.FC = () => {
  ...
  const fetchArxivPapers = async () => {
    try {
      ...
      const maxResults = 10; // Reduced to 10 for faster translation
      ...
      const parsedPapers: ResearchPaper[] = await Promise.all(Array.from(entries).slice(0, 10).map(async (entry, index) => {
        ...
        try {
          const translation = await translatePaperToArabic(title, summary.substring(0, 400));
          titleAr = translation.title;
          summaryAr = translation.summary;
        } catch (error) {
          console.error('Translation failed for paper:', title);
        }
    ...
    <section className="py-20 px-4 bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900">
      <div className="container mx-auto max-w-5xl">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <div>
            <h3 className="text-white font-bold mb-2">التصفية:</h3>
        ...
        {loading ? (
          <div className="text-center py-20">
            ...
            <p className="text-slate-400 mt-4">جاري تحميل وترجمة الأبحاث...</p>
          </div>
        ) : (
              ...
              >
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-4">
                  <div className="flex-1">
                    ...
                    {paper.titleAr && (
                      <p className="text-sm text-slate-400 mb-2 italic">{paper.title}</p>
                    )}
                  </div>
